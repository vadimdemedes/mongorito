...
class Query {
	...
	findOne(query) {
		return this.model.dbCollection()
			.then(collection => {
				return new Promise((resolve, reject) => {
					this.mquery.collection(collection).findOne(query, (err, doc) => {
						if (err) {
							reject(err);
							return;
						}
	...
	findById(id) {
		return this.model.dbCollection()
			.then(collection => {
				return new Promise((resolve, reject) => {
					this.mquery.collection(collection).findOne({ '_id': id }, (err, doc) => {
						if (err) {
							reject(err);
							return;
						}
	...
	count(query) {
		return this.model.dbCollection()
			.then(collection => {
				return new Promise((resolve, reject) => {
					this.mquery.collection(collection).count(query, (err, count) => {
						if (err) {
							reject(err);
							return;
						}
